...
bool can_show(xyz_t const& p) {
  return 0 <= p.x && p.x < RECTANGULAR_WIDTH && 0 <= p.y && p.y < RECTANGULAR_HEIGHT &&
         0 <= p.z && p.z < RECTANGULAR_DEPTH;
}
...
void repbang() {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (auto& p : ps) {
      ...
      xyz_t pos =
          q * r * RECTANGULAR_HEIGHT + xyz(RECTANGULAR_WIDTH, RECTANGULAR_HEIGHT, RECTANGULAR_DEPTH) * 0.5;
...
void spiral() {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (auto& pc : pcs) {
      double y = int((pc.p.y) * RECTANGULAR_HEIGHT + N - i * U) % RECTANGULAR_HEIGHT;
      double x =
          (pc.p.r * std::cos(pc.p.t + i * T)) * RECTANGULAR_WIDTH / 2 + RECTANGULAR_WIDTH / 2;
      double z =
          (pc.p.r * std::sin(pc.p.t + i * T)) * RECTANGULAR_WIDTH / 2 + RECTANGULAR_DEPTH / 2;
      if (can_show({x, y, z})) {
...
void brown() {
  ...
  std::deque<pos_t> poses[3]{
      ...
      {{RECTANGULAR_WIDTH / 2, RECTANGULAR_HEIGHT * 3 / 4, RECTANGULAR_DEPTH / 2}}};

  auto normalize = [](pos_t const& s) -> pos_t {
    return {(s.x + RECTANGULAR_WIDTH) % RECTANGULAR_WIDTH, (s.y + RECTANGULAR_HEIGHT) % RECTANGULAR_HEIGHT,
    ...
    for (int iy = 0; iy < RECTANGULAR_HEIGHT; ++iy) {
      for (int ix = 0; ix < RECTANGULAR_WIDTH; ++ix) {
        ...
        double iz = static_cast<int>(
            sin(x) * sin(y) * sin(i * it) * RECTANGULAR_DEPTH / 2 + RECTANGULAR_DEPTH / 2);
