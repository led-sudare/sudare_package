...
namespace sudare {
namespace {
void panic(const char* msg, int err = zmq_errno()) {
  std::stringstream ss;
...
zmq_initializer::zmq_initializer() {
  ...
  if (m_context == 0) panic("zmq_ctx_new");
}
zmq_initializer::~zmq_initializer() {
  if (zmq_ctx_shutdown(m_context) < 0) panic("zmq_ctx_shutdown");
}
...
int zmq_server::recv(char* buf, int size, int flags) const {
  ...
  if (res < 0 && zmq_errno() != EAGAIN) panic("zmq_recv");
  return res;
}
...
zmq_client::zmq_client(void* context, const char* dst) {
  ...
  if (dst[0] == '*') {
    if (zmq_bind(m_socket, tcp.c_str()) < 0) panic("zmq_bind");
  } else {
    if (zmq_connect(m_socket, tcp.c_str()) < 0) panic("zmq_connect");
  }
}
